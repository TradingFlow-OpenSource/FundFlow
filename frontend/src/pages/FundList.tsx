...
import { TrendingUp, Activity, Flame, Zap, Search } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#f0f0f0] text-black font-sans pt-20 relative overflow-x-hidden">
      ...
      <div className="bg-cyber-pink border-y-4 border-black py-2 overflow-hidden whitespace-nowrap relative z-10">
        <div className="animate-marquee inline-block font-black font-mono text-white uppercase tracking-widest text-sm">
           Warning: Degen plays only 💀 • 100x Potential detected in Sector 7 🚀 • Whale Alert: 500 ETH moved to $PEPE_AI 🐳 • Warning: Degen plays only 💀 • 100x Potential detected in Sector 7 🚀
        </div>
      </div>
      ...
      <main className="container mx-auto px-4 py-8 max-w-7xl relative z-10">
          ...
          <div className="bg-[#fff] border-4 border-black rounded-3xl p-6 md:p-8 relative shadow-[12px_12px_0px_#FACC15] transform hover:-translate-y-1 transition-transform duration-300">
            ...
            <div className="flex flex-col md:flex-row gap-8 items-center">
              ...
              <div className="w-full md:w-1/3 flex flex-col gap-4">
                  <div className="aspect-square bg-blue-200 border-4 border-black rounded-2xl flex items-center justify-center relative overflow-hidden group cursor-pointer" onClick={() => onSelect(featuredFund)}>
                    ...
                    <div className="text-9xl relative z-10 transform group-hover:scale-110 transition-transform">🐳</div>
                  </div>
              ...
              <div className="flex-1 w-full text-center md:text-left">
                <div className="flex flex-col md:flex-row justify-between items-center mb-4">
                    <h2 className="text-5xl font-black uppercase tracking-tight">{featuredFund.name}</h2>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-10">
            ...
            <div className="flex flex-wrap justify-center gap-3">
                {['🔥 Trending', '🆕 New', '💎 Blue Chip', '🐕 Memes'].map((filter) => (
                    <button 
                        key={filter}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {funds.filter(f => f.name.toLowerCase().includes(searchTerm.toLowerCase())).map((fund, i) => (
            <div 
              key={fund.id}
              onClick={() => onSelect(fund)}
              className="bg-white border-4 border-black rounded-2xl overflow-hidden relative group cursor-pointer hover:-translate-y-2 transition-transform duration-200 shadow-hard-purple"
            >
              {/* Header Color Block */}
              <div className={`h-24 border-b-4 border-black flex items-center justify-center relative overflow-hidden bg-gray-50`}>
                  ...
                  <div className="text-6xl transform group-hover:scale-125 transition-transform duration-300">
                    {fund.ticker === 'ARBIT' ? '🤖' : fund.ticker === 'SENTI' ? '🐦' : fund.ticker === 'YIELD' ? '🌾' : '🎲'}
                  </div>
              </div>
              ...
              <div className="p-5">
                <div className="flex justify-between items-start mb-2">
                    <div>
                        <h3 className="font-black text-2xl uppercase leading-none mb-1">{fund.name}</h3>
                        <div className="font-mono font-bold text-gray-500 text-sm bg-gray-100 px-2 rounded inline-block">
                            ${fund.ticker}
                        </div>
                    </div>
                    <div className={`flex flex-col items-center font-black px-2 py-1 rounded border-2 border-black shadow-[2px_2px_0px_#000] ${fund.change24h >= 0 ? 'bg-green-400' : 'bg-red-500 text-white'}`}>
                        <span className="text-xs">{fund.change24h > 0 ? '+' : ''}{fund.change24h}%</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="bg-yellow-100 border-2 border-black rounded p-2 text-center">
                        <div className="text-[10px] font-bold text-gray-500 uppercase">Vol</div>
                        <div className="font-black text-sm">{fund.volume}</div>
                    </div>
                    <div className="bg-blue-100 border-2 border-black rounded p-2 text-center">
                        <div className="text-[10px] font-bold text-gray-500 uppercase">Replies</div>
                        <div className="font-black text-sm">{fund.replies}</div>
                    </div>
                </div>

                <button className="w-full bg-black text-white font-black py-3 rounded-xl border-2 border-black hover:bg-primary hover:text-black transition-colors uppercase">
                  View Strategy
                </button>
              </div>
            </div>
